...
static void event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    if (s_retry_count < MIMI_WIFI_MAX_RETRY) {
      ...
      if (disc) {
        ESP_LOGW(TAG,
                 "Disconnected (reason=%d:%s), retry %d/%d in %" PRIu32 "ms",
...
esp_err_t wifi_manager_start(void) {
  ...
  if (nvs_open(MIMI_NVS_WIFI, NVS_READONLY, &nvs) == ESP_OK) {
    ...
    if (nvs_get_str(nvs, MIMI_NVS_KEY_SSID, (char *)wifi_cfg.sta.ssid, &len) ==
        ESP_OK) {
...
void wifi_manager_scan_and_print(void) {
  ...
  for (uint16_t i = 0; i < ap_max; i++) {
    ...
    ESP_LOGI(TAG, "  [%u] SSID=%-32s RSSI=%d CH=%d Auth=%d", i + 1,
