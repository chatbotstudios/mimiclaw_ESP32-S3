...
static void rules_save_to_spiffs(void) {
  ...
  for (int i = 0; i < MAX_MIMI_RULES; i++) {
    if (!s_rules[i].enabled && s_rules[i].id[0] == '\0')
      continue;
...
esp_err_t rules_engine_init(void) {
  ...
  if (root && cJSON_IsArray(root)) {
    ...
    for (int i = 0; i < count && i < MAX_MIMI_RULES; i++) {
      ...
      strncpy(r->id, cJSON_GetObjectItem(item, "id")->valuestring,
              RULE_ID_LEN - 1);
      strncpy(r->name, cJSON_GetObjectItem(item, "name")->valuestring,
              RULE_NAME_LEN - 1);
      strncpy(r->trigger_src, cJSON_GetObjectItem(item, "src")->valuestring,
              RULE_SRC_LEN - 1);
      ...
      strncpy(r->action_cmd, cJSON_GetObjectItem(item, "act")->valuestring,
              RULE_ACTION_LEN - 1);
      r->enabled = true;

      /* Update ID counter */
      int id_num = atoi(r->id + 2); // Skip "R_"
      if (id_num > s_rule_id_counter)
        s_rule_id_counter = id_num;
      s_rule_count++;
    }
  }
...
void rules_engine_evaluate(void) {
  ...
  mimi_sensor_read(&sd);
  float batt = mimi_power_get_battery_voltage_mv() / 1000.0f; // V

  for (int i = 0; i < MAX_MIMI_RULES; i++) {
    mimi_rule_t *r = &s_rules[i];
    if (!r->enabled || r->id[0] == '\0')
      continue;

    if (now - r->last_eval < r->interval_ms)
      continue;
    r->last_eval = now;

    float val = 0;
    if (strcmp(r->trigger_src, "temp") == 0)
      val = sd.temperature;
    else if (strcmp(r->trigger_src, "hum") == 0)
      val = sd.humidity;
    else if (strcmp(r->trigger_src, "batt") == 0)
      val = batt;
    else if (strcmp(r->trigger_src, "uptime") == 0)
      val = (float)(now / 1000);
    else
      continue;
...
esp_err_t rules_engine_add(const char *name, const char *src, mimi_cond_t cond,
                           float threshold, uint32_t interval,
                           const char *action) {
  int slot = -1;
  for (int i = 0; i < MAX_MIMI_RULES; i++) {
    if (s_rules[i].id[0] == '\0') {
