...
static const llm_provider_t *s_providers[] = {
    &anthropic_provider, &gemini_provider, &openai_provider, NULL};
...
esp_err_t llm_proxy_init(void) {
  ...
  if (!s_active_provider) {
    ESP_LOGW(TAG, "Provider '%s' not found, falling back to anthropic",
             provider_name);
...
esp_err_t llm_chat_tools(const char *system_prompt, cJSON *messages,
  ...
  esp_err_t err =
      s_active_provider->chat_tools(system_prompt, messages, tools_json, resp);

  if (err == ESP_OK) {
    /* Update metrics (TODO: usage field missing in gemini response struct)
    agent_metrics_add_tokens(resp->usage.input_tokens,
...
esp_err_t llm_set_api_key(const char *api_key) {
  ...
  if (strlen(s_active_provider->name) > 3) {
    /* shortcut for known providers to stay compatible with existing naming if
     * needed */
    if (strcmp(s_active_provider->name, "anthropic") == 0)
      strcpy(key_name, "api_key_ant");
    if (strcmp(s_active_provider->name, "gemini") == 0)
      strcpy(key_name, "api_key_gem");
    if (strcmp(s_active_provider->name, "openai") == 0)
      strcpy(key_name, "api_key_oai");
  }
...
esp_err_t llm_set_model(const char *model) {
  ...
  if (strcmp(s_active_provider->name, "anthropic") == 0)
    strcpy(key_name, "model_ant");
  if (strcmp(s_active_provider->name, "gemini") == 0)
    strcpy(key_name, "model_gem");
  if (strcmp(s_active_provider->name, "openai") == 0)
    strcpy(key_name, "model_oai");
