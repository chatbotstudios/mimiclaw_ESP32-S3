...
static esp_err_t gemini_http_direct(const char *post_data, resp_buf_t *rb,
                                    int *out_status) {
  /* Sanitize model string (replace spaces with hyphens) */
  char safe_model[64];
  strncpy(safe_model, s_model, sizeof(safe_model) - 1);
  safe_model[sizeof(safe_model) - 1] = '\0';
  for (int i = 0; safe_model[i]; i++) {
    if (safe_model[i] == ' ') safe_model[i] = '-';
  }
  ...
  snprintf(url, sizeof(url),
           ...
           safe_model, s_api_key);
           
  /* Obfuscate API key in log */
  char safe_log_url[512];
  snprintf(safe_log_url, sizeof(safe_log_url),
           "https://generativelanguage.googleapis.com/v1beta/models/"
           "%s:generateContent?key=HIDDEN",
           safe_model);
  ESP_LOGI(TAG, "Requesting Gemini URL: %s", safe_log_url);
...
static esp_err_t gemini_http_via_proxy(const char *post_data, resp_buf_t *rb,
                                       int *out_status) {
  proxy_conn_t *conn =
      proxy_conn_open("generativelanguage.googleapis.com", 443, 30000);
  if (!conn)
  ...
  int hlen =
      snprintf(header, sizeof(header),
               ...
               "Content-Length: %d\r\n"
               "Connection: close\r\n\r\n",
               safe_model, s_api_key, body_len);
               
  ESP_LOGI(TAG, "Requesting Gemini via Proxy for model: %s", safe_model);
...
static esp_err_t gemini_http_call(const char *post_data, resp_buf_t *rb,
                                  int *out_status) {
      ...
      cJSON_ArrayForEach(item, content) {
        ...
        if (strcmp(type->valuestring, "text") == 0) {
          ...
          cJSON_AddStringToObject(
              p, "text", cJSON_GetObjectItem(item, "text")->valuestring);
          cJSON_AddItemToArray(parts, p);
        } else if (strcmp(type->valuestring, "tool_use") == 0) {
          cJSON *p = cJSON_CreateObject();
          cJSON *funcCall = cJSON_CreateObject();
          cJSON_AddStringToObject(
              funcCall, "name", cJSON_GetObjectItem(item, "name")->valuestring);
          cJSON_AddItemToObject(
              funcCall, "args",
              cJSON_Duplicate(cJSON_GetObjectItem(item, "input"), 1));
          cJSON_AddItemToObject(p, "functionCall", funcCall);

          cJSON *thought_sig = cJSON_GetObjectItem(item, "thought_signature");
          if (thought_sig && cJSON_IsString(thought_sig)) {
            /* Add as both camelCase and snake_case to satisfy the API and proto */
            cJSON_AddStringToObject(p, "thoughtSignature", thought_sig->valuestring);
            cJSON_AddStringToObject(p, "thought_signature", thought_sig->valuestring);
          }

          cJSON_AddItemToArray(parts, p);
        } else if (strcmp(type->valuestring, "tool_result") == 0) {
          /* Role must be "function" for Gemini functionResponse */
          cJSON_ReplaceItemInObject(g_msg, "role",
                                    cJSON_CreateString("function"));
          cJSON *p = cJSON_CreateObject();
          cJSON *funcResp = cJSON_CreateObject();
          cJSON *name = cJSON_GetObjectItem(item, "name");
          cJSON_AddStringToObject(funcResp, "name",
                                  name ? name->valuestring : "web_search");
...
static esp_err_t gemini_init(void) {
  ...
  ESP_LOGI(TAG, "Gemini provider initialized (model: %s, endpoint: v1beta)", s_model);
  return ESP_OK;
}

static esp_err_t gemini_chat_tools(const char *system_prompt, cJSON *messages,
  ...
  if (tools_json) {
    ...
    if (in_tools && cJSON_IsArray(in_tools)) {
      ...
      cJSON_ArrayForEach(t, in_tools) {
        ...
        cJSON_AddStringToObject(decl, "name",
                                cJSON_GetObjectItem(t, "name")->valuestring);
        cJSON_AddStringToObject(
            decl, "description",
            cJSON_GetObjectItem(t, "description")->valuestring);
        cJSON_AddItemToObject(
            decl, "parameters",
            cJSON_Duplicate(cJSON_GetObjectItem(t, "input_schema"), 1));
        cJSON_AddItemToArray(func_decls, decl);
      }
    }
    cJSON_Delete(in_tools);

    cJSON_AddItemToObject(tool_decl_wrapper, "function_declarations",
                          func_decls);
    ...
    if (parts && cJSON_IsArray(parts)) {
      ...
      cJSON_ArrayForEach(p, parts) {
        ...
        if (funcCall && resp->call_count < MIMI_MAX_TOOL_CALLS) {
          ...
          cJSON *thought_sig = cJSON_GetObjectItem(p, "thought_signature");
          if (!thought_sig) thought_sig = cJSON_GetObjectItem(p, "thoughtSignature");
          if (!thought_sig) thought_sig = cJSON_GetObjectItem(funcCall, "thought_signature");
          if (!thought_sig) thought_sig = cJSON_GetObjectItem(funcCall, "thoughtSignature");

          if (thought_sig && cJSON_IsString(thought_sig)) {
            call->thought_signature = strdup(thought_sig->valuestring);
          }
