...
static bool parse_and_set_time(const char *date_str, char *out,
  ...
  ESP_LOGI(TAG, "System clock set to: %s", out);
...
static esp_err_t fetch_time_via_proxy(char *out, size_t out_size) {
  ...
  ESP_LOGI(TAG, "Proxy response headers: %s", buf);

  /* Find Date header (case-insensitive) */
  char *date_hdr = strcasestr(buf, "\r\nDate: ");
  if (!date_hdr) {
    date_hdr = strcasestr(buf, "\nDate: "); /* fallback for different EOL */
  }

  if (!date_hdr) {
    ESP_LOGE(TAG, "Date header not found in proxy response");
    return ESP_ERR_NOT_FOUND;
  }
  date_hdr = strcasestr(date_hdr, "Date: ") + 6;

  char *eol = strstr(date_hdr, "\r");
  if (!eol)
    eol = strstr(date_hdr, "\n");
  if (!eol)
    return ESP_ERR_NOT_FOUND;
  ...
  ESP_LOGI(TAG, "Found Date header: %s", date_val);
...
static esp_err_t time_http_event_handler(esp_http_client_event_t *evt) {
  if (evt->event_id == HTTP_EVENT_ON_HEADER) {
    if (strcasecmp(evt->header_key, "Date") == 0) {
      ...
      s_date_header[sizeof(s_date_header) - 1] = '\0';
    }
  }
  return ESP_OK;
}
...
static esp_err_t fetch_time_direct(char *out, size_t out_size) {
  ...
  if (s_date_header[0] == '\0') {
    ESP_LOGE(TAG, "Date header is empty or missing from response");
    esp_http_client_cleanup(client);
    return ESP_ERR_NOT_FOUND;
  }

  ESP_LOGI(TAG, "Raw Date Header captured: '%s'", s_date_header);

  if (!parse_and_set_time(s_date_header, out, out_size)) {
    esp_http_client_cleanup(client);
    return ESP_FAIL;
  }
